...
The first part will cover often overlapping concepts.

I will skip the deep dive into types and techniques of algorithmic music, so we can get to the meat of the matter:

Which is the second part: Examples of actual algorithms.
...
Events like note_on, note_off, sysex, etc.

AI such as generative adversarial networks, etc.
...
Because algorithmic music "exercises" a theory.
...
For practice and study with rhythms (i.e. a better metronome) and etudes

=item *

To make fragments to import into a larger context. Possibly with a DAW ("digital audio workstation")
...
Compositional elements: song form, tempo, key signatures, intervals, scales, etc.
...
This shows over 45 music construction modules on CPAN.

The linked ones are those mentioned in this presentation.
...
The following set of slides are complete programs.

This first one shows the score and synchronization.
...
This slide shows the sychornized parts.
...
Here we use filthy global variables! I'll be sad if you leave now though.

Music programs are not "mission critical."

And they are not "time-sensitive" either. The code just creates MIDI files, rather than real-time processing.

Anyway, here we show the Music::Scales module and show score settings.
...
First we change to the fretless bass patch on channel zero and get the scale pitches in a non-clashing scale.

Next we add four random pitches to the score, for a half-note each.
...
And here is the treble part. It uses a scale of two octaves, as opposed to the one of the previous bass routine.

And instead of half-notes, we add a quarter pitched note and a quarter-note rest.
...
But we instantiate our new chord to note list object first.
...
Generates combinatorical drum patterns
...
A lot of everything you need to add drums to a MIDI score and more.

For the GrooveTool, combinatorial patterns are "juxtaposed" = beats!
...
This Slideshow: L<https://ology.net/dev/Perl-Algorithmic-Music/>

L<On GitHub|https://github.com/ology/Perl-Algorithmic-Music>
(L<Made with S5|http://search.cpan.org/dist/Pod::S5>)

L<The GrooveTool|https://github.com/ology/Groove-Tool>

L<Algorithmic Composition Wiki|https://github.com/ology/MIDI-Perl-HOWTO/wiki>
